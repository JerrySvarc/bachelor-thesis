...
\xxx{The following summary follows the official Darklang~documentation\footnote{\url{https://docs.darklang.com/} [visited on 2024-04-22][online]}.
	...
	The editor provides the service's main functionality. It consists of a canvas that displays the created application elements as draggable boxes.
...
\xxx{One example of a programming system that demonstrates this paradigm is called \emph{Sketch-and-Sketch} \cite{sketch-and-sketch,output-directed-programming}.
	...
	After clicking on the canvas, the output is overlaid by context menus and \emph{widgets}, allowing users to modify the program's \emph{sub-expressions} and \emph{intermediate values}.
}
...
\xxx{Another programming system implementing the direct manipulation paradigm is \emph{Transformic} introduced by \citet{Schreiber_Krahn_Ingalls_Hirschfeld_2017}.
...
\xxx{This section serves as a description of web development tools used during the
	implementation of the prototype application. We will present the Elm programming language,
...
\subsection{Elm}
\xxx{\emph{Elm} is a functional programming language presented by \citet{Czaplicki_Chong_2013}.
...
\xxx{\citet{fable} is a compiler capable of compiling programs written in F\# to multiple
