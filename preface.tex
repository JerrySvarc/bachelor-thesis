...
\begin{enumerate}
	\item A simple \textbf{To-do list} application inspired by the \emph{TodoMVC benchmark}~\citet{TodoMVC}.
...
\xxx{
	...
	The user interface is inspired by \citet{darklang} and consists of a movable canvas containing various draggable elements.
	The main element is the UI modification and preview element, which shows a preview of the application's created parts alongside modification menus and is also resizable, simulating a browser window.
...
\begin{enumerate}
	...
	\item Benchmark the prototype application on the previously specified tasks.
\end{enumerate}
