...
\xxx{
	...
	we need the ability to create paths for these CustomElements as well.
	We achieve this by adding a -1 index to each Path, followed by their index inside the CustomWrapper collection.}
...
\xxx{//TODO: update replace function implementation}
\begin{listing}
...
Lastly, traverses the modified RenderingCode AST and JSON AST as described in Section~\ref{sec:traversal} and performs operations such as displaying modification menus and element preview.
...
\section{Combining RenderingCode and JSON data}
\xxx{Once the RenderingCode AST is created the system needs the ability to combine the RenderingCode elements with the data from the JSON AST.
