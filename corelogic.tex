...
We can see the different paths for each element in Program~\ref{fig:paths} which shows an example RenderingCode AST.
...
Then for each element inside a collection, we append its position inside the collection to the path.
This then allows us to traverse the AST based on the specified path and find the correct element using a recursive search funtion.
...
\xxx{
	...
	we need the ability to create paths for these CustomElements aswell.
	We achive this by adding a -1 index to each of their paths followed by their index inside the CustomWrapper collection.}

Using the dynamically created paths, we can then replace a RenderingCode by following its specified path through the RenderingCode AST.
A potential implementation of the \emph{replace} function can be seen in Program~\ref{fig:replace}

\begin{listing}
	\caption {Function used to replace a RenderingCode inside the RenderingCode AST}
	\label{fig:replace}
	\begin{lstlisting}
  \end{lstlisting}
\end{listing}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[node distance=1.5cm and 1.5cm, auto,
		...
		% Compact Legend
		\node[legend, fill=yellow!20, below right=0.5cm and 1cm of final] (user_legend) {User Operation};
