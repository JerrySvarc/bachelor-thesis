\chapter{Data-driven UI Creation - Domain and Operations}
\label{chap:corelogic}


In the previous chapter, we described the design principles which guide the implementation and functionality of the \emph{InterfaceSmith} programming system.
This chapter describes the data-driven UI creation feature of the \emph{InterfaceSmith} programming system building upon the principles described in the previous chapter.

Firstly, we describe the hole-based approach applied to the creation of elements, allowing the users to select which data to use to create UI elements.
...
The primary motivation of this approach is to implement the \emph{data-driven approach} described in the previous chapter.
Imagine you have the data shown in Figure~\ref{fig:part-json}and you wish to create a web application containing a UI element showing only the value of the \emph{Celsius} field.
...
We then define the \emph{incremental creation process} as a \emph{sequence of operations}.
